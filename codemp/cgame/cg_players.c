...
//static void CG_DoSaberLight( saberInfo_t *saber , int cnum, int bnum)
{
	...
	{//actually have something to do
		...
		trap->R_AddLightToScene(mid, 0.0-(diameter + (Q_flrand(0.0f, 1.0f)*8.0f)), rgb[0], rgb[1], rgb[2]);
	}
}
  ...
  coreradius		= ((radius * 0.4 * v2) + Q_flrand(-1.0f, 1.0f) * 0.1f)*radiusmult*cg_SFXSabersCoreSize.value;
	...
	if (end_len > 1)
	{
		...
		if (color == SABER_RGB)
		{
			...
			sbak.customShader = cgs.media.SaberBladeShader;
			saber.reType = RT_LINE;
			saber.shaderTexCoord[0] = saber.shaderTexCoord[1] = 1.0f;
			saber.shaderRGBA[0] = saber.shaderRGBA[1] = saber.shaderRGBA[2] = saber.shaderRGBA[3] = 0xff;
			saber.radius = (coreradius * AngleScale);
...
void CG_DoSaber(vec3_t origin, vec3_t dir, float length, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight, int cnum, int bnum)
{
	...
	float radiusRange;
	float radiusStart;
	vec3_t rgb = { ui_sab1_r.integer, ui_sab1_g.integer, ui_sab1_b.integer };
	int i;
	...
	{	// always add a light because sabers cast a nice glow before they slice you in half!!  or something...
 		float light = length*1.4f + (Q_flrand(0.0f, 2.0f)*3.0f);
 		CG_RGBForSaberColor(color, rgb, cnum, bnum);
					...
					switch( scolor )
					{
						...
						case SABER_RGB:
						{
										  ...
										  if (cnum < MAX_CLIENTS)
										  {
											  ...
											  if (saberNum == 0)
												  VectorSet(rgb1, ui_sab1_r.value, ui_sab1_g.value, ui_sab1_b.value);
											  else
												  VectorSet(rgb1, ui_sab2_r.value, ui_sab2_g.value, ui_sab2_b.value);
										  }
										  else
											  VectorSet(rgb1, 0.0f, 64.0f, 255.0f);
						}
